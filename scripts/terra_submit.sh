...
# (scripts/elja_submit.sh, scripts/elja_paper_benches.sh, scripts/elja_gpu_job.sh).
# Terra Slurm cluster helper only (partition cpu).
#
#   scripts/terra_submit.sh prep
#   scripts/terra_submit.sh submit
#   scripts/terra_submit.sh run      # sbatch body
set -euo pipefail
...
prep)
  ...
  pixi run -e repro -- python scripts/ecosystem_sources.py fetch \
    --root "$SOURCE_ROOT"
  pixi run -e repro -- python scripts/ecosystem_sources.py wire \
  ...
  pixi run -e repro -- python scripts/figshare_demos.py fetch data/figshare
...
submit)
  ...
  sbatch --partition="${TERRA_PARTITION:-cpu}" --exclusive \
    ...
    --output=results/terra-%j.out --wrap "scripts/terra_submit.sh run"
  ;;
run)
  cd "$ROOT"
  mkdir -p results
  ...
  pixi run -e repro -- snakemake -s workflow/Snakefile --cores all --printshellcmds
